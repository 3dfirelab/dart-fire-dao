...
#homebrewed
sys.path.append('./LoadOpticalProperties/')
...
def create_raster_dem(outputDir_simu,nx,ny,nz,dx,dy,dz):
    ...
    iio.imwrite(file_dem, dem, plugin="tifffile")
...
def setUpDartWithDAO( inputConfig, Firescene, xs, ys, zs, dxy, dz, \
    ...
    dir_current_simulation = dir_simulation+run_name+'/'
    ...
    #shutil.copytree('/home/paugam/DART_user_data/simulations/fds_burner_test_004_00_s/',dir_current_simulation)
    shutil.copy(dir_current_simulation+'/input/coeff_diff.xml', dir_current_simulation+'input/coeff_diff.xml.bak')
    shutil.copy('../data_static/'+template_dir+'/input/coeff_diff.xml', dir_current_simulation+'input/coeff_diff.xml')
    print('----------')
...
#############################
    ...
    if False: 
        ...
        parser.add_argument('-extraFDS',    '--extraFDS'    , help='flag_extraFDS'    , required=False, default= False)
        parser.add_argument('-box',         '--box'         , help='flag_box'         , required=False, default= False)
        parser.add_argument('-onecell',    '--onecell'      , help='flag oneCell'     , required=False, default= False)
        parser.add_argument('-curtainLoc', '--curtainLoc'   , help='curtain location' , required=False, default= None)
        ...
        #
        fv_thresholds = None #[str2float(xx) for xx in args.fvthreshold.split(',')]
