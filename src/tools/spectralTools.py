...
def get_tabulated_TT_Rad(srf_file, wavelength_resolution):
    ...
    for i_tt in range(b.shape[0]):
        ...
        for i_lambda, (lambda_, SRF_) in  enumerate(zip(lambda_base, SRF_base)):
            lambda__ = lambda_ + wavelength_resolution/2
            b[i_tt] += SRF_ * planck_radiance(np.array([lambda__]),np.array([TT[i_tt]]))[0] * wavelength_resolution
...
def conv_Rad2Temp(radiance, lookupTable_TTRad):
    ...
    idx = np.where( (radiance.flatten() >= lookupTable_TTRad.radiance.min()) & (radiance.flatten() <= lookupTable_TTRad.radiance.max()) ) 
    temp[idx] = interp(radiance.flatten()[idx] )
    
    idx = np.where(radiance.flatten() < lookupTable_TTRad.radiance.min() ) 
    temp[idx] = 0
    
    idx = np.where(radiance.flatten() > lookupTable_TTRad.radiance.max() ) 
    temp[idx] = np.nan
...
def planck_radiance_wn(wl,BT):
    ...
    #try:
    L = old_div(c1*wavenumber**3, ( np.exp( old_div(c2*wavenumber,BT) ) - 1 ) )
    #except: 
    #    pdb.set_trace()
    return L
...
######################################################
    ...
    srf_file = '../../data_static/Camera/optrisPI640/SpectralResponseFunction/optrisPI640.txt'
    ...
    print( conv_Rad2Temp(np.array([135.82]), lookupTable_TTRad) )
    print( 'DART gives 816.95') 
